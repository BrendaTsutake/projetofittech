    ...
    <div class="container">
        ...
        <main class="feed">
            ...
            <article class="post-card" data-post-id="<?php echo $post['post_id']; ?>">
                <div class="post-header">
                    ...
                    <a href="<?php echo $profileLink; ?>" class="profile-link">
                        <img src="<?php echo htmlspecialchars($post['profile_pic']); ?>" alt="Foto" class="user-avatar">
            ...
            <?php if (empty($feed_posts)): ?>
                <p style="text-align: center; color: #777; margin-top: 20px;">Ainda não há publicações.</p>
            <?php endif; ?>
        </main>
...
<script>
    ...
    async function carregarComentarios(postId) {
        try {
            ...
            comentarios.forEach(c => {
                ...
                // Monta o HTML do comentário
                let html = `<span class="comment-text"><strong>${c.username}</strong> ${c.texto}</span>`;

                // Se o comentário for do próprio usuário, adiciona o botão de excluir
    ...
    async function excluirComentario(commentId, postId) {
        ...
        try {
            ...
            if (result.success) {
                carregarComentarios(postId); // Recarrega a lista para sumir com o comentário
            } else {
